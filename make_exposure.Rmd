...
result                   <- st_set_crs(result, 27700)
...
summary_results <- aggregate(concentration ~ name + mode + scheme + year + pollutant + seggregated, data=final_result, FUN=mean)
...
for (i in 1:length(unique(final_result$name))) {
  ...
  plot[[i]] <- ggplot(final_result[final_result$name == scheme_name,]@data, aes(x = as.factor(year), y = concentration, fill = scheme)) +
    geom_boxplot() +
