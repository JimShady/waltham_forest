---
title: "Walking and cycling exposure in Walthamstow: 2013 v. 2021"
author: "James Smith"
output:
  html_document:
    toc: true
...
for (i in 1:length(unique(final_result$name))) {
  ...
  plot[[i]] <- ggplot(final_result[final_result$name == route_name,]@data, 
...
```{r chingford_to_leyon_spatial}

final_result <- st_as_sf(final_result)
final_result <- st_set_crs(final_result, 27700)
final_result <- st_transform(final_result, 4326)

example_2013 <- final_result[final_result$pollutant == 'no2' & final_result$name == 'chingford_to_leyton' & final_result$year == 2013,]

example_2020 <- final_result[final_result$pollutant == 'no2' & final_result$name == 'chingford_to_leyton' & final_result$year == 2020 & final_result$seggregated == 'yes',]

qpal <- colorQuantile("YlOrRd", c(example_2013$concentration, example_2020$concentration), n = 7)

leaflet()%>%
  addProviderTiles(providers$Esri.WorldTopoMap)%>%
  addFeatures(example_2013, group = '2013', fillColor = qpal(example_2013$concentration), color="transparent", fillOpacity = 0.7, label = example_2013$concentration) %>%
  addFeatures(example_2020, group = '2020', fillColor = qpal(example_2020$concentration), color="transparent", fillOpacity = 0.7, label = example_2020$concentration) %>%
